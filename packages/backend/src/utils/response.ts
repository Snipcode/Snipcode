...
interface SuccessPartial<TData extends any = any> {
  success: true
  data: TData
}
...
interface ErrorPartial<TError extends BaseError = BaseError> {
  success: false
  error: TError
}
...
const error = <TError extends BaseError = BaseError>(
  error: TError
): ErrorPartial<TError> => ({ success: false, error })
...
export { success, error, SuccessPartial, BaseError, ErrorPartial, ErrorKind }
